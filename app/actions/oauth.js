...
export function googleLogin() {
  const google = {
    ...
    clientId: '701568187599-gh7689th67hfijsclde4vsm0sftcdpuv.apps.googleusercontent.com',
...
export function link(provider) {
  ...
  default:
    return {
      type: 'LINK_FAILURE',
      messages: [{ msg: 'OAuth Provider Inválido.' }]
    }
  }
}
...
function openPopup({ url, config, dispatch }) {
  return new Promise((resolve, reject) => {
    ...
    if (url === 'about:blank') {
      popup.document.body.innerHTML = 'Carregando...';
    }
      ...
      try {
        ...
        if (popupUrlPath === redirectUriPath) {
          if (window.location.search || window.location.hash) {
            ...
            dispatch({
              type: 'OAUTH_FAILURE',
              messages: [{ msg: 'OAuth redirect ocorreu, mas não foi encontrado uma query ou hash.' }]
            });
          }
        }
      } catch (error) {
